...
удаление и push тегов на remote. Поведение моделируется по SmartGit.
...
- Annotated vs lightweight определяется только наличием message (как в SmartGit)
