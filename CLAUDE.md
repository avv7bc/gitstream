...
- В рамках этого проекта bash-команды разрешены — не спрашивать подтверждения, не задавать лишних вопросов, действовать автономно. (Разрешение задано в `.claude/settings.local.json`.)
...
- Push / Pull / Fetch (с диалогами выбора remote, таймаут сетевых операций); Fetch --prune. Pull = полный `fetch --tags <remote>` + локальный merge/rebase на `<remote>/<branch>` (`integrate_pulled`), чтобы все remote-ветки обновлялись, а не только подтягиваемая (как в SmartGit). Индикатор сетевой операции — крутилка со стрелкой направления (↑ push / ↓ pull, fetch) справа на узле репозитория в панели Repositories (`repoNetworkOps` в `useProgress.ts`); remote и обратный отсчёт — в тултипе; операция привязана к репозиторию по `repoPath`
